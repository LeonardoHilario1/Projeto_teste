...
def analise_ttest_ind(
    ...
    # Garantir apenas duas colunas
    colunas = dataframe.columns[:2]
    
    estatistica_ttest, valor_p_ttest = ttest_ind(
        dataframe[colunas[0]],
        dataframe[colunas[1]],
        equal_var=variancias_iguais,
        alternative=alternativa,   # funciona apenas em SciPy >= 1.9
        nan_policy="omit"
    )
        
    print(f"estatistica_ttest: {estatistica_ttest:.3f}")
